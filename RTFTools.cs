...
namespace HTML2OFT
{
	class RTFTools
	{
		...
		// Port from http://www.freeutils.net/source/jtnef/
		protected static uint CalculateCRC32 (byte[] buf, int off, int len)
		{
			uint c = 0;
			uint end = (uint)(off + len);
			for (uint i = (uint)off; i < end; i++)
		...
		public static string AttachRTFHeader (string raw)
		{
		...
		public static byte[] BuildCompressedRTF (string raw)
		{
			...
			int dictionaryWriteOffset = 0;
			int dictionaryEndOffset = 0;
			...
			Buffer.BlockCopy (rtfDictionary, 0, dictionaryBytes, 0, rtfDictionary.Length);
			dictionaryEndOffset = rtfDictionary.Length - 1;
			dictionaryWriteOffset = dictionaryEndOffset;

			while (runOffset < rawBytes.Length) {
				// find longest match in dictionary
				int matchOffset;
				if (dictionaryEndOffset != dictionaryBytes.Length)
					matchOffset = 0;
				else
					matchOffset = (dictionaryWriteOffset + 1) % 4096;

				int dictionaryFinalOffset = dictionaryWriteOffset % 4096;
				int bestMatchLength = 0;
				int bestMatchOffset = 0;
				int inputOffset = runOffset;

				do {
					/* TRYMATCH */
					int maxLength = 17;
					int matchLength = 0;
					int dictionaryOffset = matchOffset;

					while (matchLength < maxLength && dictionaryBytes [dictionaryOffset] == rawBytes [inputOffset]) {
						matchLength++;
						if (matchLength > bestMatchLength) {
							dictionaryBytes [dictionaryWriteOffset] = rawBytes [inputOffset];
							if (dictionaryEndOffset < dictionaryBytes.Length) {
								dictionaryEndOffset++;
							}
							dictionaryWriteOffset = (dictionaryWriteOffset + 1) % 4096;
						}
						inputOffset++;
						dictionaryOffset = (dictionaryOffset + 1) % 4096;
					}
					...
					matchOffset = (matchOffset + 1) % 4096;
				} while (matchOffset < dictionaryFinalOffset && bestMatchLength < 17);

				if (bestMatchLength == 0) {
					/* ADDBYTETODICTIONARY */
					dictionaryBytes [dictionaryWriteOffset] = rawBytes [runOffset];
					if (dictionaryEndOffset < dictionaryBytes.Length) {
						dictionaryEndOffset++;
					}
					dictionaryWriteOffset = (dictionaryWriteOffset + 1) % 4096;
					tokenBuffer [tokenOffset++] = rawBytes [runOffset++];
					controlByte |= controlBit;
					/* END ADDBYTETODICTIONARY */
				} else {
					tokenBuffer [tokenOffset++] = (byte)bestMatchOffset; // found reference start
					tokenBuffer [tokenOffset++] = (byte)(bestMatchLength - 2); // found reference length - 2 as per the spec
					runOffset = inputOffset + 1;
				}
				...
				if (controlBit != 0x80)
					...
					for (int j = 0; j < tokenOffset; j++)
						compressed.Add (tokenBuffer [j]);
					//compressed.Add(0x00); // spacer byte
					tokenOffset = 0;
					tokenBuffer = new byte[16];
					controlByte = 0;
					controlBit = 0x01;
				}    
			}

			// Wrap up with final run
			tokenBuffer [tokenOffset++] = (byte)dictionaryWriteOffset;
			tokenBuffer [tokenOffset++] = 0x00; // 0 length
			controlByte |= controlBit;
			compressed.Add (controlByte);
			for (int j = 0; j < tokenOffset; j++)
				compressed.Add (tokenBuffer [j]);
